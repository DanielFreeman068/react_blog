    ...
    return (
        <div>
            <footer>
                <div className="fcont">
                    <div className="fhead">
                    {!webName ? (<img src="https://cdn.vectorstock.com/i/preview-1x/65/30/default-image-icon-missing-picture-page-vector-40546530.jpg" alt="logo"/>) : (<header className='footerHeader'>{webName}</header>)}
                    {address ? (<div className='ftxt'>{address}</div>) : null}
                    </div>
                    <div className="links">
                        <h1>Categories</h1>
                        {routes.lifestyle? (<h3><a href={routes.lifestyle}>Lifestyle</a></h3>) : null}
                        {routes.technology? (<h3><a href={routes.technology}>Technology</a></h3>) : null}
